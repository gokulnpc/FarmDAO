...
import { AlertTriangle, Cloud, CloudLightning, CloudRain, RefreshCw } from "lucide-react"
  ...
  return (
    <div className="min-h-screen bg-neutral-800 text-white p-4 md:p-8">
      <div className="max-w-7xl mx-auto rounded-3xl bg-[#1a1a1a] overflow-hidden p-6 md:p-10">
        ...
        {/* Page Header */}
        <div className="mb-10">
          <h1 className="text-4xl font-serif mb-4">Admin / Oracle Simulation</h1>
          <p className="text-neutral-400">
            For demonstration purposes only. Simulate weather events to trigger insurance payouts.
          </p>
        </div>
        ...
        <Alert className="bg-amber-900/20 border border-amber-900/30 text-amber-400 rounded-xl mb-10">
          ...
          <AlertDescription>
            This is a simulation tool for demonstration purposes. In production, Chainlink oracles would automatically
            detect weather events and trigger payouts.
          </AlertDescription>
        </Alert>
              ...
              {simulationSuccess ? (
                <div className="text-center py-6">
                  <div className="bg-green-900/20 rounded-xl p-8 mb-6">
                    <div className="text-4xl font-bold mb-2 text-green-400">{affectedPolicies}</div>
                    <div className="text-xl mb-6">Policies Affected</div>
                    <div className="text-4xl font-bold mb-2 text-green-400">{totalPayout.toLocaleString()} FUSD</div>
                    <div className="text-xl">Total Payout Amount</div>
                  </div>

                  <p className="mb-6">
                    The simulated {selectedEvent} event in the {affectedRegion} region has triggered automatic payouts
                    to affected farmers.
                  </p>
                ...
                <div>
                  <div className="grid md:grid-cols-2 gap-6 mb-6">
                    <div>
                      <label className="block text-sm font-medium mb-2">Weather Event Type</label>
                      <Tabs defaultValue="drought" onValueChange={setSelectedEvent} className="w-full">
                        <TabsList className="bg-neutral-700 p-1 rounded-xl w-full grid grid-cols-3">
                          <TabsTrigger value="drought" className="rounded-lg data-[state=active]:bg-green-600">
                            Drought
                          </TabsTrigger>
                          <TabsTrigger value="flood" className="rounded-lg data-[state=active]:bg-green-600">
                            Flood
                          </TabsTrigger>
                          <TabsTrigger value="hail" className="rounded-lg data-[state=active]:bg-green-600">
                            Hail
                          </TabsTrigger>
                        </TabsList>
                      </Tabs>
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2">Affected Region</label>
                      <Tabs defaultValue="midwest" onValueChange={setAffectedRegion} className="w-full">
                        <TabsList className="bg-neutral-700 p-1 rounded-xl w-full grid grid-cols-3">
                          <TabsTrigger value="midwest" className="rounded-lg data-[state=active]:bg-green-600">
                            Midwest
                          </TabsTrigger>
                          <TabsTrigger value="south" className="rounded-lg data-[state=active]:bg-green-600">
                            South
                          </TabsTrigger>
                          <TabsTrigger value="west" className="rounded-lg data-[state=active]:bg-green-600">
                            West
                          </TabsTrigger>
                        </TabsList>
                      </Tabs>
                  ...
                  <div className="bg-neutral-700 rounded-xl p-6 mb-6">
                    <div className="flex items-center gap-4 mb-4">
                      {getEventIcon()}
                      <div>
                        <h3 className="text-lg font-bold capitalize">{selectedEvent} Event</h3>
                        <p className="text-sm text-neutral-400 capitalize">{affectedRegion} Region</p>
                      </div>
                    </div>
                    <p className="text-sm">
                      This will simulate a {selectedEvent} event in the {affectedRegion} region. The oracle will detect
                      the event and automatically trigger payouts to affected farmers based on their policy coverage.
                    </p>
                  </div>
                  ...
                  <div className="bg-neutral-700/50 rounded-xl p-6 mb-6">
                    ...
                    <ol className="list-decimal list-inside space-y-2 text-sm">
                      <li>Oracle detects weather event data from multiple sources</li>
                      <li>Smart contract verifies the data against policy conditions</li>
                      <li>If conditions are met, automatic payouts are triggered</li>
                      <li>Farmers receive FUSD directly to their wallets</li>
                      <li>Transaction is recorded on the blockchain for transparency</li>
                    </ol>
                  </div>
                </div>
              )}
